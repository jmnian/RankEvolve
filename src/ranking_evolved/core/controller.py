...
class Controller:
    ...
    async def run(self, *, seed_path: Path, resume: bool = False) -> Program:
        if resume:
            ...
            self._persist_program(
                seed_program, prompt_system=None, prompt_user=None, llm_raw=None,
                iteration=0, parent_metrics=None, eval_duration=0.0,
            )
            ...
            self._append_program_row(
                program=seed_program, parent=None, iteration=0, generation=0,
            )
            start_iteration = 1
    ...
    async def _step(self, iteration: int) -> None:
        ...
        previous_programs = self.strategy.top_programs(n=3, island_idx=parent_island)
        ...
        # Apply diff (fail-loudly). Wrap to a DiffApplication regardless.
        try:
            child_code, diff_app = apply_search_replace(parent.source_code, candidate.raw_response)
        except DiffError as exc:
            self.logger.warning(
                "iteration %d: diff did not apply (%s); skipping admit.",
                iteration, exc.application.fatal_error,
            )
            self._record_skipped_iteration(
                iteration=iteration, parent=parent, prompt=prompt,
                candidate=candidate, diff=exc.application, snap_before=snap_before,
            )
            return
        ...
        eval_result, child_program = await self._evaluate_child(
            child_id=child_id, child_code=child_code, parent=parent, iteration=iteration,
        )

        # Admit + record DB-state diff.
        best_before = self.strategy.best_program_id
        admission = self.strategy.admit(child_program, iteration=iteration)
        is_new_best = (
            self.strategy.best_program_id == child_program.id
            and best_before != child_program.id
        )
        ...
        self._persist_program(
            ...
            diff_summary={
                "n_extracted": diff_app.n_extracted, "n_applied": diff_app.n_applied,
            },
    ...
    async def _build_and_eval_seed(self, seed_path: Path) -> Program:
        ...
        # When latency-aware: extract per-dataset seed latencies as the
        # baseline that all later candidates are compared against, persist
        # them, and recompute the seed's `combined_score` under the
        # configured objective so it sits on the same scale as the children.
    ...
    async def _evaluate_child(
        self, *, child_id: str, child_code: str, parent: Program, iteration: int,
        ...
        if (
            ...
            outcome = compute_objective(
                per_ds, self._baseline_latency_by_dataset, self.config.objective,
            )
            _merge_outcome_into_metrics(metrics, outcome, self.config.objective)
        ...
        self._append_program_row(
            program=program, parent=parent, iteration=iteration,
    ...
    def _log_iteration_progress(
        ...
        *,
        ...
        marker = " NEW BEST FOUND!" if is_new_best else ""
        self.logger.info(
    ...
    def _record_skipped_iteration(
        self, *, iteration: int, parent: Program, prompt, candidate, diff: DiffApplication,
        snap_before,
    ) -> None:
        ...
        with self.store.transaction() as conn:
            ...
            self.store.set_meta("strategy_state", self.strategy.state_dict(), conn=conn)
        self.trace.append(
            iteration=iteration, parent_id=parent.id, child_id=None,
            parent_metrics=parent.metrics, child_metrics=None,
            improvement_delta=None,
            prompt={"system": prompt.system, "user": prompt.user}
            if self.config.trace.include_prompts else None,
...
def _explode_per_dataset(flat_metrics: dict[str, Any]) -> dict[str, dict[str, float]]:
    ...
    for key in flat_metrics:
        for suf in _PER_DATASET_SUFFIXES:
            if key.endswith(suf) and not key.startswith("avg_") and not key.startswith("total_"):
