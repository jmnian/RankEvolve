...
# Metric cutoffs.
#
# `RECALL_K` doubles as the `top_k` passed to `bm25.rank(...)` so that the
# retrieval depth matches the recall cutoff and the timed phase does the same
# work the metric is computed on.
#
# Both are configurable per-run via env vars (set by the controller from
# `Config.objective`); defaults reproduce the legacy `recall@100, ndcg@10`
# behavior so existing scripts that just import `evaluate(...)` keep working.
NDCG_K = int(os.environ.get("EVAL_NDCG_K", "10"))
RECALL_K = int(os.environ.get("EVAL_RECALL_K", "100"))

# Number of queries to run untimed before the timed phase (per dataset).
# Default 0 preserves legacy semantics (no warmup, no per-query timing).
WARMUP_QUERIES = int(os.environ.get("EVAL_WARMUP_QUERIES", "0"))

# Objective name. The evaluator only uses this to decide whether to write the
# legacy `combined_score` formula (for `recall100_ndcg10`) vs. leaving it as 0
# for the controller to recompute (for any other objective). Per-dataset and
# aggregate raw metrics are always emitted.
OBJECTIVE_NAME = os.environ.get("EVAL_OBJECTIVE_NAME", "recall100_ndcg10")
...
class DatasetResult:
    ...
    `recall_at_100` is the legacy field name kept for backward compatibility;
    when `RECALL_K != 100` the value here is recall@RECALL_K and the
    aggregator emits the correct `recall@<k>` key (see `aggregate_results`).
    `query_latency_median_ms` is the per-query median measured during the
    timed phase (warmup excluded, indexing excluded).
    """
    name: str
    ndcg_at_10: float
    recall_at_100: float
    ...
    # Per-query latency statistics (populated whenever WARMUP_QUERIES > 0).
    # Indexing time is reported separately and is NEVER mixed in here.
    query_latency_median_ms: float | None = None
    num_timed_queries: int = 0
    num_warmup_queries: int = 0
...
def _run_query_phases(
    ...
    *,
    ...
    """Run warmup (untimed) + timed retrieval phases for one dataset.

    Returns `(all_relevant, all_retrieved, per_query_latency_ms, n_warmup,
    n_timed)`. Indexing is done by the caller; this function never touches
    the index, so its timing only covers `tokenize_fn(q)` + `bm25.rank(...)`.

    Fairness rule: warmup uses the FIRST `warmup_queries` queries, the
    timed phase uses the REMAINING queries. If we have too few queries
    (`len < 2*warmup_queries`), we fall back to "warm up on the first N,
    then time the FULL set" so tiny datasets still produce a usable
    median; this is intentionally the same rule for every candidate.

    Metric computation (relevant-doc lookup, list bookkeeping) happens
    AFTER `t1 = perf_counter()` so it is never folded into the query
    latency. Indexing time is never included here.
    """
    n = len(queries)
    if warmup_queries <= 0 or n == 0:
        warmup_n = 0
        timed_start = 0
    else:
        warmup_n = min(warmup_queries, n)
        # Fallback: dataset too small to give the timed phase enough room.
        if n < 2 * warmup_queries:
            timed_start = 0  # time the whole set after warming up
        else:
            timed_start = warmup_n

    # Warmup phase — no timing, no metric collection.
    for i in range(warmup_n):
        q_tokens = tokenize_fn(queries[i])
        bm25.rank(q_tokens, top_k=recall_k)
    ...
    for i in range(timed_start, n):
        qid = query_ids[i]
        query_text = queries[i]
        t0 = time.perf_counter()
        q_tokens = tokenize_fn(query_text)
        ranked_indices, _ = bm25.rank(q_tokens, top_k=recall_k)
    ...
    return all_relevant, all_retrieved, per_query_latency_ms, warmup_n, timed_n
...
def evaluate_single_dataset(
    ...
    try:
        ...
        # Warmup + timed retrieval (per-query latency captured in the timed phase).
        all_relevant, all_retrieved, per_query_latency_ms, n_warm, n_timed = (
            _run_query_phases(
                bm25, tokenize_fn,
                list(query_ids), list(queries), dataset, id_to_idx,
                recall_k=RECALL_K, warmup_queries=WARMUP_QUERIES,
            )
        )
        ...
        query_latency_median_ms = (
            float(np.median(per_query_latency_ms)) if per_query_latency_ms else None
        )
        ...
        if not all_relevant:
            return DatasetResult(
                ...
                num_queries=0,
                error="No valid queries",
                query_latency_median_ms=query_latency_median_ms,
                num_timed_queries=n_timed,
                num_warmup_queries=n_warm,
            )
...
def _evaluate_queries_on_index(
    ...
    # Warmup + timed retrieval — see _run_query_phases for fairness rules.
    all_relevant, all_retrieved, per_query_latency_ms, n_warm, n_timed = (
        _run_query_phases(
            bm25, tokenize_fn,
            list(query_ids), list(queries), dataset, id_to_idx,
...
# when the framework loads this file as "evaluation_module" (child processes would otherwise
# fail with ModuleNotFoundError: No module named 'evaluation_module').
from tasks.bm25.evaluator_worker import _worker_evaluate
...
def aggregate_results(results: list[DatasetResult]) -> dict[str, Any]:
    ...
    for result in results:
        ...
        # Per-dataset metrics. We always emit `recall@<RECALL_K>` (the
        # configured cutoff) AND keep writing the legacy `recall@100` key
        # when RECALL_K==100 so existing dashboards / tools don't break.
        output[f"{prefix}_{ndcg_key}"] = result.ndcg_at_10
        if ndcg_key != "ndcg@10":
            output[f"{prefix}_ndcg@10"] = result.ndcg_at_10  # legacy alias
        output[f"{prefix}_{recall_key}"] = result.recall_at_100
        if recall_key != "recall@100":
            output[f"{prefix}_recall@100"] = result.recall_at_100  # legacy alias
        output[f"{prefix}_index_time_ms"] = result.index_time_ms
        output[f"{prefix}_query_time_ms"] = result.query_time_ms
        if result.query_latency_median_ms is not None:
            output[f"{prefix}_query_latency_median_ms"] = result.query_latency_median_ms
        ...
        if result.error:
            ...
            if result.query_latency_median_ms is not None:
                all_latencies.append(result.query_latency_median_ms)
    ...
    output[f"avg_{ndcg_key}"] = avg_ndcg
    if ndcg_key != "ndcg@10":
        output["avg_ndcg@10"] = avg_ndcg  # legacy alias
    output[f"avg_{recall_key}"] = avg_recall
    if recall_key != "recall@100":
        output["avg_recall@100"] = avg_recall  # legacy alias

    if all_latencies:
        output["avg_query_latency_median_ms"] = float(np.mean(all_latencies))

    # `combined_score`: legacy formula only when the run is using the
    # legacy objective. For any other objective the controller recomputes
    # this from raw metrics + baseline latencies after evaluation.
    if OBJECTIVE_NAME == "recall100_ndcg10":
        output["combined_score"] = 0.0 if datasets_failed > 0 else (0.8 * avg_recall + 0.2 * avg_ndcg)
    else:
        output["combined_score"] = 0.0
        output["combined_score_pending"] = 1.0
    ...
    output["recall_k"] = float(RECALL_K)
    output["ndcg_k"] = float(NDCG_K)
    output["warmup_queries"] = float(WARMUP_QUERIES)
