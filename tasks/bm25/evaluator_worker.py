"""
Worker entrypoint for ProcessPoolExecutor used by tasks/bm25/evaluator.py.

This module exists so that when the framework loads tasks/bm25/evaluator.py
under a different name (e.g. "evaluation_module"), spawned worker processes can still
unpickle the task: the task references tasks.bm25.evaluator_worker._worker_evaluate,
...
def _worker_evaluate(args: tuple) -> "DatasetResult | list[DatasetResult]":
    ...
    from tasks.bm25.evaluator import (
        EvalConfig,
        DatasetResult,
