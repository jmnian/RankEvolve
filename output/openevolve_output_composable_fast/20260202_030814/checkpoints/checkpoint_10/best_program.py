...
class EvolvedParameters:
    ...
    k3: float = 8.0
    delta: float = 0.5
    alpha: float = 0.6   # BM25F-style blend weight: raw BM25 vs. normalized term salience
    beta: float = 0.35   # coverage/coordination strength
    gamma: float = 0.05  # small rarity shaping for very rare terms
    epsilon: float = 1e-9
...
class ScoringPrimitives:
    ...
    def idf_balanced(df: float, N: int) -> float:
        """
        New primitive: a bounded, two-sided IDF that reduces over-boosting ultra-rare terms
        while still penalizing frequent terms. Uses log1p for stability.
        """
    ...
    def tf_salience(tf: float, dl: float, avgdl: float) -> float:
        """
        New primitive: document-length-aware term salience (tf share with a mild length stabilizer).
        Intended to help long docs where raw tf can be inflated.
        """
    ...
    def coord_factor(matched_terms: int, total_query_terms: int, beta: float) -> float:
        """
        New primitive: smooth coordination factor (soft AND).
        Multiplies score by (matched/total)^beta to reward broader query coverage without
        killing recall as hard-AND would.
        """
...
class TermScorer:
    ...
    def score(tf: float, df: float, N: int, dl: float, avgdl: float) -> float:
        ...
        # IDF: use bounded "balanced" IDF to reduce extreme rare-term spikes (helps robustness).
        ...
        # TF part: blend BM25-like saturation with a length-aware salience term.
        ...
        # Mild "rare-term shaping": sublinear boost only for very high-idf terms.
...
class DocumentScorer:
    ...
    def score(
        ...
        # Coordination multiplier (soft AND) to improve early precision (nDCG@10) while
        # typically preserving recall@100 better than additive coverage bonuses.
...
class BM25:
    ...
    def _score_candidates_vectorized(
        ...
        # Apply soft coordination factor using matched term count per doc.
        if EvolvedParameters.beta > 0:
            ...
            coord = np.power(np.maximum(matched / max(total_q, 1.0), 0.0), EvolvedParameters.beta)
            scores *= coord
