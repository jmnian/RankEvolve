...
class Config:
    ...
    # NEW: let ultra-common terms become *negative* evidence (repulsion).
    # Intuition: if df/N is high, seeing the term is weak/negative evidence of relevance,
    # improving top-rank precision in boilerplate-heavy corpora.
    use_signed_common_idf: bool = True
    common_neg_strength: float = 0.22   # magnitude of negative contribution at df≈N
    common_neg_power: float = 1.3       # curvature vs df/N

    # NEW: bounded IDF-mass coverage reward (soft-AND over information).
    # Uses matched_mass / total_mass, both computed over shaped IDF.
...
def retrieval_score(
    ...
    for term in query_repr.terms:
        ...
        if df >= common_thr:
            ...
            # Signed common-term evidence: very common terms can become *negative*.
            if Config.use_signed_common_idf:
                # smooth in [0,1] increasing with df; 0 at threshold, 1 near N
                u = min(1.0, (df - common_thr) / (float(N) - common_thr + eps))
                q_idf -= Config.common_neg_strength * (u ** Config.common_neg_power)
    ...
    for term in query_repr.terms:
        ...
        # For coverage, only count positive (discriminative) mass.
        if term_idf > 0.0:
