...
class Config:
    ...
    # NEW: "aboutness" / salience reward.
    # Intuition: a relevant doc is often *about* the query, not just mentioning terms.
    # Measure: fraction of doc tokens that are query tokens (query-term mass / doc_length).
    # Keep it bounded (multiplier >= 1) so recall@100 is typically preserved.
    salience_alpha: float = 0.18   # strength; 0 disables
    salience_power: float = 0.5    # concave (sqrt) to avoid over-rewarding short docs
...
def retrieval_score(
    ...
    # Track query-token mass in this doc for salience/aboutness.
    q_mass = 0.0
    ...
    # NEW: Salience/aboutness reward (bounded, recall-safe).
    # Helps long-document corpora (e.g., SciDocs, StackOverflow, TREC-COVID) where many docs
    # match a few query terms incidentally; prefers docs where query terms occupy more mass.
...
class BM25:
    ...
    def _score_candidates_vectorized(
        ...
        # Accumulate query-token mass for salience (vectorized).
        ...
        # NEW: Salience/aboutness reward (vectorized mirror of retrieval_score()).
        if Config.salience_alpha > 0.0 and len(query_term_ids) > 0:
            # q_mass is the total count of matched query tokens in the document.
            # We can compute it from tf rows already read (tfparts are tf-saturated, so use tf directly).
            # Re-read tf is expensive; instead accumulate while iterating terms.
            pass

        # Salience/aboutness reward (must mirror retrieval_score()).
        ...
        if q_idf_max > 0.0 and Config.anchor_mix_alpha > 0.0:
            ...
            # Apply same salience to anchor channel before mixing (mirror retrieval_score()).
            if Config.salience_alpha > 0.0:
                anchor_scores *= sal
