...
3. Run evolution (config currently set to 200 iterations):  
   `python openevolve/openevolve-run.py src/ranking_evolved/bm25.py evaluator_bright_biology.py --config openevolve_config.yaml`
...
### Scoring equations (current kernel)

- Smoothed IDF:  
  `idf(t) = log((N + 1) / (df(t) + ε))`, with `ε ≈ 1e-5`.
- Per-term contribution for a query term `t` with frequency `tf` in document `d`:  
  `score(t, d) = idf(t) * ( tf / (tf + k1 + 1) ) * ( tf * (k1 + 1) / ( tf + k1 * (1 - b + b * |d| / avg_dl) ) )`  
  where `k1=1.5`, `b=0.75`, `|d|` is document length, and `avg_dl` is average document length.  
  The document score is the sum over query terms. An early exit returns 0 when no query terms appear in the document.
